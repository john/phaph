namespace :dropbox do
  ...
  def extract_files(client, path, files)
    directory = client.metadata(path)
    
    directory['contents'].each do |file_info|
      puts "file_info['is_dir']: #{file_info['is_dir']}"
  ...
  task :get_files => :environment do
    files = []
    client = DropboxClient.new( 'thHXL1FwB6IAAAAAAAATItVkNWyuYfplLzo1dhm-0nPH1BS9b2LtalPCqwqNZIs-' )
